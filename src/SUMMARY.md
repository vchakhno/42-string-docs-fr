# Summary

[Introduction](README.md)

- [Les chars](chars.md)
	- [Les character literals](chars/character-literals.md)
	- [L'arithmétique](chars/arithmetic.md)
	- [Séquence d'échappement](chars/escape-sequence.md)
	- [Signé/non-signé](chars/signed-unsigned.md)
	- [La table ASCII](chars/ascii.md)
- [Les bases des strings](strings-basics.md)
- [Storage and lifetime](todo.md)
	- [Les segments de données](data-segments/todo.md)
		- [.text](data-segments/todo.md)
		- [.data](data-segments/todo.md)
		- [.bss](data-segments/todo.md)
		- [La heap](data-segments/heap.md)
		- [La stack](data-segments/todo.md)